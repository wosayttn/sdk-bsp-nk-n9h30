...
static void SDH_IRQHandler(int vector, void *param)
{
    ...
    else if ((ier & SDH_INTEN_CDIEN1_Msk) && (isr & SDH_INTSTS_CDIF1_Msk))  // card number=1 detect
    {
#if defined(NU_SDH_HOTPLUG)
...
static void nu_card_detector(nu_sdh_t sdh)
{
...
#if (NU_SDH_CARD_DETECTION == CardDetect_From_DAT3)
    if (!SDH_CardDetection(sdh_base, sdh->info, sdh->card_num))
    {
#else
    isr = sdh_base->INTSTS;
    ...
    else
    {
        if (!sdh->IsCardMounted)
        {
            ...
            SDH_CardSelect(sdh->base, sdh->info, sdh->card_num);
#endif
            SDH_Open(sdh_base, sdh->info, NU_SDH_CARD_DETECTION | sdh->card_num);
...
static void sdh_hotplugger(void *param)
{
    ...
    for (i = (SDH_START + 1); i < SDH_CNT; i++)
    {
        ...
        SDH_Open(nu_sdh_arr[i].base, nu_sdh_arr[i].info, NU_SDH_CARD_DETECTION | nu_sdh_arr[i].card_num);
        if (!SDH_Probe(nu_sdh_arr[i].base, nu_sdh_arr[i].info, nu_sdh_arr[i].card_num) &&
                nu_sdh_arr[i].info->IsCardInsert)
        {
            nu_sdh_arr[i].IsCardMounted = (nu_sdh_hotplug_mount(&nu_sdh_arr[i]) == RT_EOK) ? 1 : 0 ;
        }
